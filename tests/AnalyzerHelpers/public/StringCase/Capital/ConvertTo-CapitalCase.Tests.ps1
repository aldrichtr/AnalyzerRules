...
$options = @{
    Tag  = @( 'unit', 'StringCase', 'ConvertTo', 'CapitalCase')
    Name = 'GIVEN the public function ConvertTo-CapitalCase'
    Foreach = $sourceFile
}
Describe @options {
    ...
    Context 'WHEN The function is sourced in the current environment' {
        ...
        It 'THEN it should load without error' {
            (Get-Command 'ConvertTo-CapitalCase') | Should -Not -BeNullOrEmpty
        }
