...
$options = @{
    Tag  = @( 'unit', 'Corrections', 'Initialize', 'CorrectionCollection')
    Name = 'GIVEN the public function Initialize-CorrectionCollection'
    Foreach = $sourceFile
}
Describe @options {
    ...
    Context 'WHEN The function is sourced in the current environment' {
        ...
        It 'THEN it should load without error' {
            (Get-Command 'Initialize-CorrectionCollection') | Should -Not -BeNullOrEmpty
        }
