...
$options = @{
    Tag  = @( 'unit', 'Corrections', 'New', 'PSScriptAnalyzerCorrection')
    Name = 'GIVEN the public function New-PSScriptAnalyzerCorrection'
    Foreach = $sourceFile
}
Describe @options {
    ...
    Context 'WHEN The function is sourced in the current environment' {
        ...
        It 'THEN it should load without error' {
            (Get-Command 'New-PSScriptAnalyzerCorrection') | Should -Not -BeNullOrEmpty
        }
