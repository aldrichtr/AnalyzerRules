...
$options = @{
    Tag  = @( 'unit', '', 'Measure', 'OperatorLowerCase')
    Name = 'GIVEN the public function Measure-OperatorLowerCase'
    Foreach = $sourceFile
}
Describe @options {
    ...
    Context 'WHEN The function is sourced in the current environment' {
        ...
        It 'THEN it should load without error' {
            (Get-Command 'Measure-OperatorLowerCase') | Should -Not -BeNullOrEmpty
        }
