...
$options = @{
    Tag  = @( 'unit', '', 'Measure', 'ParamKeywordLowerCase')
    Name = 'GIVEN the public function Measure-ParamKeywordLowerCase'
    Foreach = $sourceFile
}
Describe @options {
    ...
    Context 'WHEN The function is sourced in the current environment' {
        ...
        It 'THEN it should load without error' {
            (Get-Command 'Measure-ParamKeywordLowerCase') | Should -Not -BeNullOrEmpty
        }
