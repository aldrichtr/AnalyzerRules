...
$options = @{
    Tag  = @( 'unit', '', 'Measure', 'ParameterAttributeIsFalse')
    Name = 'GIVEN the public function Measure-ParameterAttributeIsFalse'
    Foreach = $sourceFile
}
Describe @options {
    ...
    Context 'WHEN The function is sourced in the current environment' {
        ...
        It 'THEN it should load without error' {
            (Get-Command 'Measure-ParameterAttributeIsFalse') | Should -Not -BeNullOrEmpty
        }
