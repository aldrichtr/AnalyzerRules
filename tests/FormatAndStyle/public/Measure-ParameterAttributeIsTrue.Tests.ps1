...
$options = @{
    Tag  = @( 'unit', '', 'Measure', 'ParameterAttributeIsTrue')
    Name = 'GIVEN the public function Measure-ParameterAttributeIsTrue'
    Foreach = $sourceFile
}
Describe @options {
    ...
    Context 'WHEN The function is sourced in the current environment' {
        ...
        It 'THEN it should load without error' {
            (Get-Command 'Measure-ParameterAttributeIsTrue') | Should -Not -BeNullOrEmpty
        }
