...
$options = @{
    Tag  = @( 'unit', '', 'Measure', 'NamedBlockLowerCase')
    Name = 'GIVEN the public function Measure-NamedBlockLowerCase'
    Foreach = $sourceFile
}
Describe @options {
    ...
    Context 'WHEN The function is sourced in the current environment' {
        ...
        It 'THEN it should load without error' {
            (Get-Command 'Measure-NamedBlockLowerCase') | Should -Not -BeNullOrEmpty
        }
