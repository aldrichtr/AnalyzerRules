...
$options = @{
  Tag     = @( 'unit', 'StringCase', 'Test', 'TrainCase')
  Name    = 'given the private function Test-TrainCase'
  Foreach = $sourceFile
}
Describe @options {
  ...
  Context 'WHEN The function is sourced in the current environment' {
    ...
    It 'THEN it should load without error' {
      (Get-Command 'Test-TrainCase') | Should -Not -BeNullOrEmpty
    }
