...
$options = @{
  Tag     = @( 'unit', 'StringCase', 'Test', 'CamelCase')
  Name    = 'given the private function Test-CamelCase'
  Foreach = $sourceFile
}
Describe @options {
  ...
  Context 'WHEN The function is sourced in the current environment' {
    ...
    It 'THEN it should load without error' {
      (Get-Command 'Test-CamelCase') | Should -Not -BeNullOrEmpty
    }
