...
$options = @{
  Tag     = @( 'unit', 'DiagnosticRecord', 'New', 'DiagnosticRecord')
  Name    = 'given the private function New-DiagnosticRecord'
  Foreach = $sourceFile
}
Describe @options {
  ...
  Context 'WHEN The function is sourced in the current environment' {
    ...
    It 'THEN it should load without error' {
      (Get-Command 'New-DiagnosticRecord') | Should -Not -BeNullOrEmpty
    }
