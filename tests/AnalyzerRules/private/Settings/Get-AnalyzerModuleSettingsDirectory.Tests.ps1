...
$options = @{
  Tag     = @( 'unit', 'Settings', 'Get', 'AnalyzerModuleSettingsDirectory')
  Name    = 'given the private function Get-AnalyzerModuleSettingsDirectory'
  Foreach = $sourceFile
}
Describe @options {
  ...
  Context 'WHEN The function is sourced in the current environment' {
    ...
    It 'THEN it should load without error' {
        (Get-Command 'Get-AnalyzerModuleSettingsDirectory') | Should -Not -BeNullOrEmpty
    }
