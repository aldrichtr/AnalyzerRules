...
$options = @{
  Tag     = @( 'unit', 'Settings', 'Get', 'AnalyzerSettingPreset')
  Name    = 'given the private function Get-AnalyzerSettingPreset'
  Foreach = $sourceFile
}
Describe @options {
  ...
  Context 'WHEN The function is sourced in the current environment' {
    ...
    It 'THEN it should load without error' {
      (Get-Command 'Get-AnalyzerSettingPreset') | Should -Not -BeNullOrEmpty
    }
